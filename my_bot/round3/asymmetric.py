...
class Trader:
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            result[product] = self.hydro_beast_v27_logic(state, state.order_depths[product], memory)

        return result, conversions, json.dumps(memory)

    def hydro_beast_v27_logic(self, state: TradingState, depth: OrderDepth, memory: dict) -> List[Order]:
        orders: List[Order] = []
        product = "HYDROGEL_PACK"
        ...
        # --- CONFIGURATION (MAX ASYMMETRIC SIZING) ---
        LIMIT = 200
        MAX_CHUNK_TAKER = 20 
        BASE_MAKER_SIZE = 190 # Pushed to the absolute limit
        ...
        # 3. RELAXED INVENTORY SKEW 
        # Slightly more relaxed since asymmetric sizing protects us from double-fills
        if abs(current_pos) > 160:
            skew_multiplier = ((abs(current_pos) - 160) / 40.0) ** 2
            skew_sign = 1 if current_pos > 0 else -1
            inv_skew = skew_sign * skew_multiplier * 10 + (current_pos / 75.0)
        else:
        ...
        if best_ask < adjusted_fair - 2.0 and buy_cap > 0:
            qty = min(buy_cap, abs(depth.sell_orders[best_ask]), MAX_CHUNK_TAKER)
            orders.append(Order(product, best_ask, qty))
            buy_cap -= qty
            
        if best_bid > adjusted_fair + 2.0 and sell_cap < 0:
            qty = max(sell_cap, -depth.buy_orders[best_bid], -MAX_CHUNK_TAKER)
            orders.append(Order(product, best_bid, qty))
            sell_cap -= abs(qty)

        # 5. ASYMMETRIC SIZING
        pos_ratio = current_pos / float(LIMIT) # ranges from -1 to 1
        
        # Exponential scaling for safety (e.g., if we have 100 pos, pos_ratio is 0.5, we want to buy even less)
        bid_multiplier = math.exp(-2.0 * pos_ratio) if pos_ratio > 0 else (1 - pos_ratio)
        ask_multiplier = math.exp(2.0 * pos_ratio) if pos_ratio < 0 else (1 + pos_ratio)

        bid_size = int(BASE_MAKER_SIZE * bid_multiplier)
        ask_size = int(BASE_MAKER_SIZE * ask_multiplier)
        
        # Clamp to reasonable values
        bid_size = max(5, min(195, bid_size))
        ask_size = max(5, min(195, ask_size))

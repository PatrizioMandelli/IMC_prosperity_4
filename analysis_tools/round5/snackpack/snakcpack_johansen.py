...
def analyze_basket(name, group, prices):
    """Johansen + OLS spread analysis; prints lot-sizing config."""
    ...
    for target in group:
        ...
        print(f"    R²: {model.rsquared:.6f}  ADF p: {adf_p:.6f}  HL: {hl:.1f}")
    ...
    print(f"\n{'─' * 60}")
    print(f"BEST: {best['target']}  R²={best['r2']:.6f}  ADF p={best['adf_p']:.6f}  HL={best['hl']:.1f}")
    print(f"lots = {lots}")
    print(f"'{name}': {{")
...
if __name__ == '__main__':
    ...
    analyze_basket('SNACKPACK_DUO', ['SNACKPACK_CHOCOLATE', 'SNACKPACK_VANILLA'], prices)
    analyze_basket('SNACKPACK_TRIO',
                   ['SNACKPACK_PISTACHIO', 'SNACKPACK_RASPBERRY', 'SNACKPACK_STRAWBERRY'], prices)

...
Steps:
  1. Single-asset mean reversion test (ADF, Hurst, OU half-life).
  2. All pairwise OLS spreads: p1 - (alpha + beta * p2).
  3. Structural baskets with fixed unit weights (replication candidates).
  4. Each panel regressed on the basket of the remaining four.

Output: stdout table + analysis_tools/round5/panel/pairs_output.txt
"""
...
def analyze_basket(name, weights, prices):
    """Constructs spread = sum_i(w_i * mid_i) and runs stationarity analysis."""
...
def main():
    ...
    print(f"Loaded {len(prices)} ticks across days "
          f"{sorted(prices.index.get_level_values('day').unique())}")

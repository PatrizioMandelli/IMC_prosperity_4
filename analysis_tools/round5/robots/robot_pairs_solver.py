...
def analyze_pair(p1, p2, prices):
    ...
    # Half-life via OU regression on spread
...
def analyze_basket(target, basket, prices):
    """Regress target on basket of other assets."""
    cols = [target] + basket
    if any(c not in prices.columns for c in cols):
        return None
    data = prices[cols].dropna()
...
def main():
    ...
    for b in basket_results:
        ...
        print(f"{b['target']:<20} {b['r2']:>6.3f} {b['adf_p']:>8.4f} {b['spread_std']:>8.2f} {hl:>8}")
    ...
    # Print best stationary pairs config
    print(f"\n{'='*100}")

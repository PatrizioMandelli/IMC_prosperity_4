...
def read_day_data(file_reader: FileReader, round_num: int, day_num: int, no_names: bool) -> BacktestData:
    ...
    with file_reader.file([f"round{round_num}", f"prices_round_{round_num}_day_{day_num}.csv"]) as file:
        ...
        # Memory dictionary to hold the last valid price per product
        last_valid_mid = {}

        for line in lines:
            columns = line.split(";")
            product = columns[2]
            raw_mid = float(columns[15])

            # PURE FILTER: If it is an actual price, update our memory.
            # If it is 0, we completely ignore it and use the memory.
            if raw_mid > 0:
                last_valid_mid[product] = raw_mid

            # Safely grab the price (defaults to 10k only for the very first tick if broken)
            clean_price = last_valid_mid.get(product, 10000.0)
            ...
            prices.append(
                PriceRow(
                    ...
                    mid_price=clean_price,  # Inject the perfectly smooth price
    ...
    with file_reader.file([f"round{round_num}", f"trades_round_{round_num}_day_{day_num}.csv"]) as file:
        if file is not None:
            for line in file.read_text(encoding="utf-8").splitlines()[1:]:
                ...
                trades.append(
                    Trade(
                        symbol=columns[3], price=int(float(columns[5])), quantity=int(columns[6]),
                        buyer=columns[1], seller=columns[2], timestamp=int(
                            columns[0]),
                    )
                )
    ...
    with file_reader.file([f"round{round_num}", f"observations_round_{round_num}_day_{day_num}.csv"]) as file:
        if file is not None:
            for line in file.read_text(encoding="utf-8").splitlines()[1:]:
                ...
                observations.append(
                    ObservationRow(
                        timestamp=int(columns[0]), bidPrice=float(columns[1]), askPrice=float(columns[2]),
                        transportFees=float(columns[3]), exportTariff=float(columns[4]), importTariff=float(columns[5]),
                        sugarPrice=float(columns[6]), sunlightIndex=float(columns[7]),
                    )
                )
